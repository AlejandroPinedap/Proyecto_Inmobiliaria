defmodule Inmobiliaria.Server do
  ...
  defp handle_input(input, session) do
    case Map.get(session, :context) do
      ...
      :publishing_rooms -> handle_publish_rooms(input, session)
      :publishing_area -> handle_publish_area(input, session)
      ...
      :filter_modality -> handle_filter_by_modality(input, session)
  ...
  defp handle_register_user(username, session) do
    ...
    if exists do
      ...
      new_session =
        session |> Map.put(:context, :registering_pass) |> Map.put(:reg_username, username)
  ...
  defp handle_register_pass(password, session) do
    new_session =
      session |> Map.put(:context, :registering_role) |> Map.put(:reg_password, password)
  ...
  defp handle_client_menu(option, session) do
    case option do
      "1" ->
        show_properties_with_filter(session)

      "2" ->
        {"Ingresa el ID de la propiedad a comprar:", Map.put(session, :context, :buying_id)}

      "3" ->
        {"Ingresa el ID de la propiedad a arrendar:", Map.put(session, :context, :renting_id)}

      "4" ->
        {"Ingresa el ID de la propiedad:", Map.put(session, :context, :messaging_id)}

      "5" ->
        show_inbox(session)

      "6" ->
        {"Ingresa el usuario al que quieres escribir:",
         Map.put(session, :context, :chatting_user)}

      "7" ->
        show_score(session)

      "8" ->
        show_ranking(session)
  ...
  defp handle_owner_menu(option, session) do
    case option do
      "1" ->
        {"Ingresa el ID de la propiedad:", Map.put(session, :context, :publishing_id)}

      "2" ->
        show_my_properties(session)
  ...
  defp handle_publish_type(type, session) do
    ...
    if String.downcase(type) not in valid do
      ...
      new_session =
        session
        |> Map.put(:context, :publishing_modality)
        |> Map.put(:pub_type, String.downcase(type))
  ...
  defp handle_publish_modality(modality, session) do
    ...
    if clean not in ["venta", "arriendo"] do
      ...
      new_session =
        session |> Map.put(:context, :publishing_city) |> Map.put(:pub_modality, clean)
  ...
  defp handle_publish_price(price_str, session) do
    case Integer.parse(String.trim(price_str)) do
      ...
      {price, _} ->
        new_session =
          session
          |> Map.put(:context, :publishing_rooms)
          |> Map.put(:pub_price, price)

        {"Numero de habitaciones:", new_session}
    end
  end

  defp handle_publish_rooms(rooms_str, session) do
    case Integer.parse(String.trim(rooms_str)) do
      :error ->
        {"[ERROR] Ingresa un numero valido:", session}

      {rooms, _} ->
        new_session =
          session
          |> Map.put(:context, :publishing_area)
          |> Map.put(:pub_rooms, rooms)

        {"Area en m2:", new_session}
    end
  end

  defp handle_publish_area(area_str, session) do
    case Integer.parse(String.trim(area_str)) do
      :error ->
        {"[ERROR] Ingresa un numero valido:", session}
      ...
      {area, _} ->
        data = %{
          ...
          price: Map.get(session, :pub_price),
          rooms: Map.get(session, :pub_rooms),
          area: area,
        ...
        clean_session =
          ...
          |> Map.delete(:pub_city)
          |> Map.delete(:pub_price)
          |> Map.delete(:pub_rooms)
  ...
  defp handle_chat_user(target, session) do
    case ClientRegistry.lookup(target) do
      ...
      {:ok, _} ->
        new_session =
          session |> Map.put(:context, :chatting_text) |> Map.put(:chat_target, target)
  ...
  defp show_score(session) do
    ...
    case UserManager.get_score(username) do
      {:ok, score} ->
        {"[PTS] #{username}: #{score} puntos\r\n" <> show_main_menu(session.role), new_session}

      {:error, _} ->
        {"[ERROR] Error consultando puntaje\r\n" <> show_main_menu(session.role), new_session}
    end
  end

  defp show_ranking(session) do
    new_session = Map.put(session, :context, :main_menu)
    case UserManager.ranking_all() do
      [] ->
  ...
  defp show_history(session) do
    new_session = Map.put(session, :context, :main_menu)
    content = Inmobiliaria.Operations.OperationLogger.get_history()
  ...
  defp handle_filter_option(option, session) do
    ...
    case String.trim(option) do
      "1" ->
        {"Tipo (casa, apartamento, oficina, lote):", Map.put(clean, :context, :filter_type)}

      "2" ->
        {"Ciudad:", Map.put(clean, :context, :filter_city)}

      "3" ->
        {"Precio minimo:", Map.put(clean, :context, :filter_price_min)}

      "4" ->
        {"Modalidad (venta / arriendo):", Map.put(clean, :context, :filter_modality)}
      ...
      _ ->
        {"[ERROR] Opcion invalida. Elige 0-5:", Map.put(clean, :context, :filter_menu)}
    end
  end
  ...
  defp handle_filter_price_min(min_str, session) do
    ...
    case Integer.parse(String.trim(min_str)) do
      {min, _} ->
        {"Precio maximo:",
         clean |> Map.put(:context, :filter_price_max) |> Map.put(:price_min, min)}
  ...
  defp handle_filter_price_max(max_str, session) do
    ...
    ns =
      session |> Map.delete(:context) |> Map.delete(:price_min) |> Map.put(:context, :main_menu)
    ...
    case Integer.parse(String.trim(max_str)) do
      ...
      :error ->
        {"[ERROR] Precio invalido:",
         session |> Map.put(:context, :filter_price_max) |> Map.put(:price_min, min)}
    end
  end

  defp handle_filter_by_modality(modality, session) do
    ns = Map.put(session, :context, :main_menu)
    results = PropertyManager.search_by_modality(String.trim(modality))
    if Enum.empty?(results) do
      {"No hay propiedades de modalidad #{modality}\r\n" <>
      show_main_menu(session.role),ns}
    else
      {format_properties_table(results) <> "\r\n" <> show_main_menu(session.role), ns}
    end
  ...
  defp format_properties_table(properties) do
    ...
    header =
      separator <>
        "\r\n ID          | Tipo      | Modalidad | Ciudad    | Precio      |Hab | Area | Estado     | Dueno" <>
        separator

    rows =
      properties
      |> Enum.map(fn p ->
        " #{String.pad_trailing(p.id, 10)} | " <>
          "#{String.pad_trailing(p.type, 9)} | " <>
          "#{String.pad_trailing(p.modality, 9)} | " <>
          "#{String.pad_trailing(p.city, 9)} | " <>
          "#{String.pad_trailing("$#{p.price}", 11)} | " <>
          "#{String.pad_trailing("#{p.rooms}", 3)} | " <>
          "#{String.pad_trailing("#{p.area}m2", 5)} | " <>
          "#{p.status}"
      end)
      |> Enum.join("\r\n")
